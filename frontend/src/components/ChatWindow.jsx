 ...
 * Handles the main conversational interface, message history, and deep-dive analysis triggering.
 * * @param {Function} onNewMessage - Callback triggered when a new message is successfully sent.
 * @param {Object} theme - The current active UI theme palette.
...
const ChatWindow = ({ onNewMessage, theme, sessionId }) => {
  ...
  const submitMessage = async (textToSend) => {
    ...
    const userMsg = { userMessage: textToSend, aiResponse: "...", role: "temp" };
  ...
  const handleDeepAnalysis = async (msg, index) => {
    ...
    try {
      ...
      alert("Failed to analyze context. Please try again.");
    } finally {
  ...
  const activeTheme = theme || {
    bgBase: "#0f172a", surface: "#1e293b", border: "#334155", accentRed: "#E60000", textMain: "#f8fafc", textMuted: "#94a3b8",
  };
  ...
  return (
    ...
    >
          ...
          {messages.length === 0 && !loading && (
            <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", animation: "fadeIn 0.5s ease" }}>
              <div style={{ backgroundColor: activeTheme.surface, padding: "16px", borderRadius: "20px", marginBottom: "20px", boxShadow: isLightMode ? "0 10px 30px rgba(0,0,0,0.05)" : "0 10px 30px rgba(0,0,0,0.2)" }}>
                <img 
                  src={isLightMode ? "/logo.jpg" : "/logo_dark.png"} 
                  alt="RupeeLetter Logo" 
                  style={{ height: "48px", width: "auto", borderRadius: "10px" }} 
                  onError={(e) => e.target.style.display = 'none'} 
                />
              </div>
              <h2 style={{ fontSize: "28px", fontWeight: "800", color: activeTheme.textMain, marginBottom: "10px", letterSpacing: "-0.5px", textAlign: "center" }}>
                How can I help you today?
              </h2>
              <p style={{ color: activeTheme.textMuted, fontSize: "16px", marginBottom: "40px", textAlign: "center", maxWidth: "500px", lineHeight: "1.5" }}>
                Ask me about stock updates, financial news, or deep-dive market analysis.
              </p>
                ...
                {suggestions.map((suggestion, idx) => (
                  <button
                    ...
                    style={{
                      ...
                      boxShadow: isLightMode ? "0 4px 10px rgba(0,0,0,0.03)" : "none"
                    }}
                    onMouseEnter={(e) => {
                      e.currentTarget.style.borderColor = activeTheme.accentRed;
                      e.currentTarget.style.transform = "translateY(-2px)";
                      e.currentTarget.style.boxShadow = isLightMode ? "0 10px 20px rgba(230,0,0,0.1)" : "0 10px 20px rgba(0,0,0,0.3)";
                    }}
                    onMouseLeave={(e) => {
                      e.currentTarget.style.borderColor = activeTheme.border;
                      e.currentTarget.style.transform = "translateY(0)";
                      e.currentTarget.style.boxShadow = isLightMode ? "0 4px 10px rgba(0,0,0,0.03)" : "none";
                    }}
                  >
                ...
                <div style={{ backgroundColor: activeTheme.surface, color: activeTheme.textMain, padding: "15px 20px", borderRadius: "18px 18px 18px 0", maxWidth: "85%", lineHeight: "1.6", border: `1px solid ${activeTheme.border}`, boxShadow: isLightMode ? "0 4px 15px rgba(0,0,0,0.03)" : "0 4px 15px rgba(0,0,0,0.1)" }}>
                  ...
                  {m.sources && m.sources.length > 0 && (
                    <div style={{ marginTop: "12px", display: "flex", gap: "8px", flexWrap: "wrap" }}>
                      {m.sources.map((s, si) => (
                        <a key={si} href={s.source} target="_blank" rel="noreferrer" style={{ fontSize: "11px", color: activeTheme.textMain, textDecoration: "none", display: "flex", alignItems: "center", gap: "4px", backgroundColor: activeTheme.bgBase, padding: "4px 10px", borderRadius: "6px", border: `1px solid ${activeTheme.border}`, transition: "0.2s ease", opacity: 0.8 }} onMouseEnter={(e) => { e.currentTarget.style.opacity = 1; e.currentTarget.style.borderColor = activeTheme.accentRed; }} onMouseLeave={(e) => { e.currentTarget.style.opacity = 0.8; e.currentTarget.style.borderColor = activeTheme.border; }}>
      ...
      {/* Input Module */}
      <div style={{ padding: "20px", borderTop: `1px solid ${activeTheme.border}`, transition: "background-color 0.3s ease, border-color 0.3s ease", backgroundColor: activeTheme.bgBase }}>
        <style>
          {`
        ...
        <form onSubmit={handleSend} style={{ maxWidth: "850px", margin: "0 auto", display: "flex", gap: "10px", backgroundColor: isLightMode ? "#FFFFFF" : activeTheme.surface, padding: "12px", borderRadius: "20px", border: `1px solid ${isLightMode ? "#CBD5E1" : activeTheme.border}`, boxShadow: isLightMode ? "0 10px 30px rgba(0,0,0,0.08)" : "0 10px 30px rgba(0,0,0,0.3)", transition: "all 0.3s ease" }}>
          <input className="theme-input" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask about stock updates or business news..." style={{ flex: 1, background: "transparent", border: "none", color: activeTheme.textMain, outline: "none", fontSize: "15px", padding: "8px 12px", transition: "color 0.3s ease" }} />
