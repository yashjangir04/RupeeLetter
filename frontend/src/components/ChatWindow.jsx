...
const ChatWindow = ({ onNewMessage, theme, sessionId }) => {
  ...
  const [sessionLoading, setSessionLoading] = useState(false); // New state for session transitions
  ...
  useEffect(() => {
    const loadSessionChats = async () => {
      ...
      // Optional: Add a minimum fake delay to prevent "flicker" on fast connections
      const minDelay = new Promise(resolve => setTimeout(resolve, 600));

      try {
        const fetchTask = getSession(sessionId);
        const [response] = await Promise.all([fetchTask, minDelay]);
  ...
  const submitMessage = async (textToSend) => {
    ...
    setMessages((prev) => [...prev, { userMessage: textToSend, aiResponse: null, role: "temp" }]);
    setLoading(true);
    setInput("");
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", backgroundColor: activeTheme.bgBase, height: '100%', overflow: 'hidden', position: "relative" }}>
      ...
      <style>
        {`
          ...
          .typing-dot {
            ...
            height: 6px;
            margin: 0 2px;
            background-color: ${activeTheme.textMuted};
            border-radius: 50%;
      ...
      {/* 🟢 FULL WINDOW SESSION LOADER */}
      {sessionLoading && (
        <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: activeTheme.bgBase, zIndex: 10, animation: "fadeIn 0.3s ease" }}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "15px" }}>
             <Loader2 size={40} className="animate-spin" style={{ color: activeTheme.accentRed }} />
             <span style={{ color: activeTheme.textMuted, fontSize: "14px", fontWeight: "500", letterSpacing: "0.5px" }}>LOADING CONVERSATION...</span>
          </div>
        </div>
      )}
      ...
      <div className="custom-scrollbar" style={{ flex: 1, overflowY: "auto", padding: "20px", display: "flex", flexDirection: "column" }}>
        <div style={{ maxWidth: "850px", margin: "0 auto", width: "100%", flex: 1, display: "flex", flexDirection: "column" }}>
          ...
          {!sessionLoading && messages.map((m, i) => (
            <div key={i} style={{ marginBottom: "30px", animation: "fadeIn 0.3s ease" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start" }}>
                <div style={{ 
                    ...
                    boxShadow: isLightMode ? "0 4px 15px rgba(0,0,0,0.03)" : "0 4px 15px rgba(0,0,0,0.1)",
                    animation: m.role === "temp" ? "pulseSkeleton 1.5s infinite ease-in-out" : "none" 
                }}>
                  ...
                  {m.aiResponse === null ? (
                    <div style={{ display: "flex", flexDirection: "column", gap: "8px", width: "200px" }}>
                      ...
                      <div style={{ height: "8px", backgroundColor: activeTheme.border, borderRadius: "4px", width: "100%" }}></div>
                      <div style={{ height: "8px", backgroundColor: activeTheme.border, borderRadius: "4px", width: "60%" }}></div>
                    </div>
                  ) : (
                    <>
                      {m.aiResponse}
                      ...
                      {m.role !== "temp" && (
                        <div style={{ marginTop: "15px", borderTop: `1px solid ${activeTheme.border}`, paddingTop: "10px" }}>
                          <button 
          ...
          {loading && !sessionLoading && (
            <div style={{ color: activeTheme.textMuted, fontSize: "14px", display: "flex", alignItems: "center", gap: "10px", padding: "10px 0", animation: "fadeIn 0.3s ease" }}>
              <Loader2 size={16} className="animate-spin" style={{ color: activeTheme.accentRed }} />
              <span style={{ fontWeight: "500" }}>Analyzing market data...</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div style={{ padding: "20px", borderTop: `1px solid ${activeTheme.border}`, backgroundColor: activeTheme.bgBase }}>
        <form onSubmit={(e) => { e.preventDefault(); submitMessage(input); }} style={{ maxWidth: "850px", margin: "0 auto", display: "flex", gap: "10px", backgroundColor: isLightMode ? "#FFFFFF" : activeTheme.surface, padding: "12px", borderRadius: "20px", border: `1px solid ${isLightMode ? "#CBD5E1" : activeTheme.border}`, boxShadow: isLightMode ? "0 10px 30px rgba(0,0,0,0.08)" : "0 10px 30px rgba(0,0,0,0.3)" }}>
          ...
          <button type="submit" disabled={loading || sessionLoading || !input.trim()} style={{ backgroundColor: activeTheme.accentRed, border: "none", padding: "12px 18px", borderRadius: "12px", cursor: (loading || sessionLoading || !input.trim()) ? "not-allowed" : "pointer", opacity: (loading || sessionLoading || !input.trim()) ? 0.5 : 1, transition: "all 0.2s ease", display: "flex", alignItems: "center", justifyContent: "center" }}>
